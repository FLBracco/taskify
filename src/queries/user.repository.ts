...
import { LoginInput, UserDbType} from "../models/login.models.js";
...
export async function findUser(name: string | undefined, email:string | undefined): Promise <UserDbType | null>{
    try {
        const findUserQuery = `
            SELECT * FROM users
            WHERE name = $1 OR email = $2; 
